     ...
     <div class="type_title">管理员信息 </div>
      <div class="xinxi">
        <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">用户名： </label>
          <div class="col-sm-9"><input type="text" name="用户名" id="website-title" value="${pname} " class="col-xs-7 text_info" disabled="disabled">
          &nbsp;&nbsp;&nbsp;<a href="javascript:ovid()" onclick="change_Password()" class="btn btn-warning btn-xs">修改密码</a></div>
          <input type="hidden" id="pa" value="${pass}"/>
          <input type="hidden" id="idd" value="${pid}"/>
          </div>
 ...
 //初始化宽度、高度    
  ...
  function change_Password(){
	...
	yes:function(index, layero){		
          ...
          /* alert($("#pa").val()+"A");
          alert($("#password").val()+"B"); */
		   if ($("#pa").val()!=$("#password").val()){
				  layer.alert('原密码不不正确!',{
	              title: '提示框',				
					icon:0,
		    ...
		    if(!$("#c_mew_pas").val || $("#c_mew_pas").val() != $("#Nes_pas").val() )
        {
            layer.alert('密码不一致!',{
			 ...
			 $.ajax({
	       		   ...
	       		   data: {password:$("#c_mew_pas").val(),id:$("#idd").val()},
